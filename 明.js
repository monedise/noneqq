...
let subConfig = "https://raw.githubusercontent.com/monedise/noneqq/refs/heads/main/ACL4SSR_Online_Mini_MultiMode.ini";
...
let FileName = atob('U2FsdGVkX1+hapJgCzs1wjteHrMi83vN');
let BotToken;
let ChatID; 
let proxyhosts = [];
let proxyhostsURL = '';
    ...
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== 'websocket') {
        ...
        if (路径 == '/') {
          ...
          const UD = Math.floor(((now - today.getTime())/86400000) * 999 * 1099511627776 / 2);
          let pagesSum = UD;
          let workersSum = UD;
          let total = 0.01 * 1099511627776 ;
...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, env) {
  if (sub) {
    ...
    addresses = addresses.concat('127.0.0.1:1234#nonat');
    if (hostName.includes(".workers.dev")) {
      addressesnotls = addressesnotls.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#none节点'));
    } else {
      addresses = addresses.concat(cfips.map(cidr => generateRandomIPFromCIDR(cidr) + '#none节点'));
    }
  }
  ...
  if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
    ...
    let 订阅器 = '\n';
    if (sub) {
      if (enableSocks) 订阅器 += `NOCDN（访问方式）: Socks5\n  ${newSocks5s.join('\n  ')}\n${socks5List}`;
      else if (proxyIP && proxyIP != '') 订阅器 += `NOCDN（访问方式）: ProxyIP\n  ${proxyIPs.join('\n  ')}\n`;
      else if (RproxyIP == 'true') 订阅器 += `NOCDN（访问方式）: 自动获取ProxyIP\n`;
      else 订阅器 += `NOCDN（访问方式）: 无法访问, 需要您设置 proxyIP/PROXYIP ！！！\n`
      订阅器 += `\nSUB（优选订阅生成器）: ${sub}`;
    } else {
      if (enableSocks) 订阅器 += `NOCDN（访问方式）: Socks5\n  ${newSocks5s.join('\n  ')}\n${socks5List}`;
      else if (proxyIP && proxyIP != '') 订阅器 += `NOCDN（访问方式）: ProxyIP\n  ${proxyIPs.join('\n  ')}\n`;
